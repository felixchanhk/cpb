...
function cpb_output_custom_field() {
    ...
    <div class="form-group">
        ...
        <input v-model="vLength" type="text" class="form-control cpb_number" id="iLength" name="nLength" value="259" placeholder="100mm - 400mm" onchange="check_not_null()" required>
    ...
    <div class="form-group">
        ...
        <input v-model="vWidth" type="text" class="form-control cpb_number" id="iWidth" name="nWidth" value="95" placeholder="40mm - 200mm" onchange="check_not_null()" required>
    ...
    <div class="form-group">
        ...
        <input v-model="vHeight" type="text" class="form-control cpb_number" id="iHeight" name="nHeight" value="259" placeholder="100mm - 450mm" onchange="check_not_null()" required>
...
function cpb_add_custom_field_to_cart_item($cart_item_data, $product_id, $variation_id) {
    ...
    if (!empty($nPrint)) {
    	...
    	$cart_item_data['iPreviewImage'] = '<img src="http://47.74.226.159/file/file/composingPreview/'.get_order_preview_image().'" width="35%" height="auto"/>';
    }
	...
	//$product_id = $cart_item['product_id'];
        ...
        if(!empty($cart_item['ncpb_file'])) {
    		...
    		if(in_array($ext,$allowed) ) {
    			$img = '<img src="http://47.74.226.159/cpb_uat/cpb_die_cut_upload/'.$cart_item['ncpb_file'].'" />';
			}else{
    			$img = '<img src="http://47.74.226.159/cpb_uat/wp-content/uploads/2017/11/shopping-bag-white-mock-up-1.jpg" alt="'.$ext.'"/>';
        	}
    	}else{
    		$img = '<img src="http://47.74.226.159/cpb_uat/wp-content/uploads/2017/11/shopping-bag-white-mock-up-1.jpg" />';
    	}
...
function edit_online_now_link() {
    ...
    $form_html .= '<a class="cpb_delivery_options" href="' .  get_permalink() . '/shipping-and-devlivery/" target="_blank">See delivery options>></a></div></div>';
	
	$form_html .= '<a class="cpb_custome_qoute" href="' .  get_permalink() . '/contact-us/">Custom Qoute</a>';
    ...
    $return_string = '<div class="container cpb-shipping-container" id="cpb-shipping-cost-table">
            ...
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12">
                    ...
                    <p class="cpb-shipping-notices">*For customer reference only. MakePlayingCards.com cannot guarantee the delivery standards of the postal services. You may expect a longer delivery time of additional 3-5 days on top of the normal transit time for remote areas.</p>
                </div>
            </div>
        </div>';
...
function add_custom_weight($cart_object) {
    ...
    foreach ($cart_object->get_cart() as $cart_item) {
        ...
        if ($cart_item['data']->get_sku() == 'MDSB7569') {
            ...
            $cart_item['data']->set_weight($productWeight);
        }
    }
}
...
function wcw_cart() {
    ...
    if (WC()->cart->needs_shipping()) :
        ...
        <tr class="shipping">
            ...
            <td><strong><span class="woocommerce-Price-amount amount"><?php echo number_format($woocommerce->cart->cart_contents_weight,3) . ' kg'; ?></span></strong></td>
        </tr>
    <?php
    endif;
}
...
function conditional_guest_checkout_based_on_product( $value ) {
  $restrict_ids = array( 738 ); // Replace with product ids which cannot use guest checkout

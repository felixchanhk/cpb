...
function cpb_api_file_path() {
    return '/var/www/html/cpb_uat/';
}

function get_order_preview_image() {
    $file = cpb_api_file_path().'/wp-content/api/productInstanceId.txt';
    ...
    $file = cpb_api_file_path().'/wp-content/api/image_path.txt';
...
function check_order_duplicated($order_id) {
    $file = cpb_api_file_path().'/wp-content/api/completeOrderId.txt';
...
function mysite_woocommerce_order_status_completed($order_id) {
    ...
    //$file = '/var/www/html/customprintbox/wp-content/uploads/productInstanceId.txt';
    $file = cpb_api_file_path().'/wp-content/api/productInstanceId.txt';
...
//$data_string = json_encode($data);
    ...
    $file = cpb_api_file_path().'/wp-content/api/order_log.txt';
    ...
    $file = cpb_api_file_path().'/wp-content/api/order_info.txt';
    $myfile = fopen($file, "w") or die("Unable to open order_log file!");
    $txt = $data_string;
    fwrite($myfile, $txt);
    fclose($myfile);
...
function post_cproduct_id_api($Length, $Width, $Height, $iEditPrint, $iEditPaperType, $iEditFinish, $iEditHandleType, $iEditHandleColor, $iEditHandleLength, $token) {
    ...
    $ch = curl_init(api_gateway() . '/cgp-rest/api/products/179388/skuProduct'); //179388 sz give us spu id
    ...
    //$file = '/var/www/html/customprintbox/wp-content/uploads/productid.txt';
    $file = cpb_api_file_path().'/wp-content/api/productid.txt';
...
function dynamic_size_pId() {
    ...
    if ($iEditHandleType == "Ploypropylene") {
        $iEditHandleType = 178346;
    } else {
        $iEditHandleType = 178347;
    }

    $iEditHandleColor = $_GET["iEditHandleColor"];
    if ($iEditHandleColor == "Black") {
        $iEditHandleColor = 178349;
    } else {
        $iEditHandleColor = 178350;
    }

    $iEditHandleLength = $_GET["iEditHandleLength"];
    if ($iEditHandleLength == "450mm One Side") {
        $iEditHandleLength = 178352;
    } else if ($iEditHandleLength == "550mm One Side") {
        $iEditHandleLength = 178353;
    } else {
        $iEditHandleLength = 178354;
    }
 ...
 */
function get_token() {
    $file = cpb_api_file_path().'/wp-content/api/output_token.txt';

...
function fpd_output_admin_notice( $type, $headline, $message, $condition=true, $name=null, $dismissable=false ) {
	...
	if( $condition && empty($dismiss_option) ) {
		...
		?>
		<div class="notice notice-<?php echo $type; ?> fpd-dismiss-notification">
			<?php if($dismissable) echo '<button class="notice-dismiss" value="' . $name . '"></button>'; ?>
			<?php if($headline) echo '<h4>' . $headline . ' </h4>'; ?>

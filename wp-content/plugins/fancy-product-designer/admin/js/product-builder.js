jQuery(document).ready(function($) {
	...
	$modalEditViewOptions.find('.fpd-save-admin-modal').click(function() {
		...
		//save old bleed box
		if(currentViewOptions.bleed_box) {
			newValues.bleed_box = currentViewOptions.bleed_box;
		}
		...
		if(currentViewOptions.output_width !== newValues.output_width || currentViewOptions.output_height !== newValues.output_height) {
			...
			if(newValues.output_width !== undefined && newValues.output_height !== undefined) {
				...
				_updateBleedBox(pxWidth, pxHeight);

			}

			if(bleedBoxRect) { //save new bleed box coords

				var bbBoundingRect = bleedBoxRect.getBoundingRect();
				Object.keys(bbBoundingRect).forEach(function(key) {
					bbBoundingRect[key] = parseInt(bbBoundingRect[key]);
				});

				newValues.bleed_box = bbBoundingRect;
				bleedBoxChanged = true;

			}
			else { // width or height is not set, remove bleed box
				newValues.bleed_box = null;
			}
			...
			success: function(data) {
				...
				if(!_.isUndefined(data) && !_.isUndefined(data.columns)) {
					...
					if(!_.isUndefined(data.columns.options)) {
						...
						if(bleedBoxChanged && $bleedBoxToolbar.is(':hidden')) {
							$('#fpd-edit-bleed-box').click();
						}
	...
	.on('productCreate', function() {
		...
		if(fpdGlobalProductBuilderOpts.bleedBox) {

			var bleedBox = fpdGlobalProductBuilderOpts.bleedBox;
			_updateBleedBox(bleedBox.width, bleedBox.height, bleedBox.left, bleedBox.top);
		...
		stage.on({
			...
			'object:moving': function(opts) {
				...
				if(opts.target.name === 'bleed-box' && bleedBoxRect) {

					$bleedBoxToolbar.find('[name="left"]').val(parseInt(bleedBoxRect.left));
					$bleedBoxToolbar.find('[name="top"]').val(parseInt(bleedBoxRect.top));
			...
			'object:scaling': function(opts) {
				...
				if(opts.target.name === 'bleed-box' && bleedBoxRect) {
					$bleedBoxToolbar.find('[name="scale"]').val(bleedBoxRect.scaleX);
					$bleedBoxToolbar.find('#fpd-bleed-box-pixels').html(parseInt(bleedBoxRect.getWidth())+ ' x ' + parseInt(bleedBoxRect.getHeight()) +' pixels');
				}
	...
	var $bleedBoxToolbar = $('#fpd-bleed-box-toolbar'),
		bleedBoxOptionsLoaded = false;

	$('#fpd-edit-bleed-box').click(function() {

		if($bleedBoxToolbar.is(':hidden')) {

			toggleModalLoader($bleedBoxToolbar, true);
			$.ajax({
				url: fpd_admin_opts.adminAjaxUrl,
				data: {
					action: 'fpd_loadviewoptions',
				...
				success: function(data) {
					...
					if(data && data.options) {
						...
						bleedBoxRect.evented = true;
						stage.setActiveObject(bleedBoxRect);

					}

					toggleModalLoader($bleedBoxToolbar);

				}
			});

			$bleedBoxToolbar.addClass('fpd-show');
		}
		else {

			bleedBoxRect.evented = false;
			stage.discardActiveObject().renderAll();
			$bleedBoxToolbar.removeClass('fpd-show');

		}

	});

	$bleedBoxToolbar.find('input').change(function() {

		if(bleedBoxRect) {

			if(this.name === 'scale') {
				bleedBoxRect.scaleX = parseFloat(this.value);
				bleedBoxRect.scaleY = parseFloat(this.value);
				$bleedBoxToolbar.find('#fpd-bleed-box-pixels').html(parseInt(bleedBoxRect.getWidth())+ ' x ' + parseInt(bleedBoxRect.getHeight()) +' pixels');
			}
			else {
				bleedBoxRect[this.name] = parseInt(this.value);
			}

			bleedBoxRect.setCoords();
			stage.renderAll();
	...
	$('#fpd-save-bleed-box-options').click(function() {
		...
		var optionsData = $.extend({}, currentViewOptions, {bleed_box: bbBoundingRect});
		optionsData = fpdIsEmptyObject(optionsData) ? '' : optionsData;
	...
	var _updateBleedBox = function(width, height, left, top) {
		...
		if(width && height) {
			...
			stage.add(bleedBoxRect)

			if(_.isNumber(top) && _.isNumber(left)) { // set existing bleed box

				bleedBoxRect.setLeft(left);
				bleedBoxRect.setTop(top);

			}
			else { //set new bleed box and scale to fit in canvas and center

				if(bbWidth > bbHeight) {
					if(bbWidth > viewInstance.options.stageWidth) {

						var scale = (viewInstance.options.stageWidth * 0.8) / bbWidth;
						bleedBoxRect.setWidth(viewInstance.options.stageWidth * 0.8);
						bleedBoxRect.setHeight(scale * bbHeight);
					...
					if(bbHeight > viewInstance.options.stageHeight) {
						...
						bleedBoxRect.setHeight(viewInstance.options.stageHeight * 0.8);
						bleedBoxRect.setWidth(scale * bbWidth);
			...
			$('#fpd-edit-bleed-box').removeClass('radykal-disabled');
			...
			$('#fpd-edit-bleed-box').addClass('radykal-disabled');
